...
The standard is what changes how Claude behaves, and that is what you turn on per project.
...
(project conventions) with you, which is what a copied file leaves undone. It will not overwrite a
standard the project already has.
...
the actual files (with `file:line` you can open), then makes you explain it back and corrects you
honestly, because being told is not the same as understanding. It also runs the other way: explain a
part to it and it checks you against the code. It is read-only, so it surfaces problems for you to
decide on rather than fixing them mid-lesson. Good before a supervision meeting, a defense, or
...
`skills/` is the single source of truth for both install paths, so there is no second copy to keep
in sync.
